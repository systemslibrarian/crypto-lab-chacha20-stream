 ...
 *
 *  - text over a surface axe declines to resolve. This page is full of them.
 *    Every tinted band is a translucent `rgba()` laid on a themed surface —
 *    the XOR exhibit's three rows (`rgba(88,166,255,.12)`,
 *    `rgba(210,153,34,.12)`, `rgba(63,185,80,.12)`), the four state-matrix
 *    word tints, `--accent-dim`, the amber `.primer-note` and the red
 *    `.warning-banner` — and the hero aside is a `color-mix(in oklab, …)`
 *    Chromium hands back unresolved, as is `.cl-hero-why-label`'s ink. axe
 *    files those nodes under `incomplete` rather than measuring them, so a
 *    gate that asserts only on `violations` never sees them. That is exactly
 *    where this lab's contrast defect lived: the three XOR rows are FIXED
 *    tints authored against the dark `--bg-input`, and in the light theme the
 *    same 12% wash over near-white left the accent / amber / green glyphs on
 *    top of it at 3.96–4.22:1.
 ...
 *    at `.85` and every `:disabled` control at `.4` (the latter exempt as an
 *    inactive component, and skipped below for that reason). The gate this
 *    replaces measured neither.
 *
 ...
 *    clip live text: `.xor-viz` around a grid one column per plaintext byte,
 *    `#qr-step-table` around a 500px-min operation table, and `.table-scroll`
 *    around the 22rem state-matrix legend. Content scrolled past any of them is
 ...
 *    ancestor's box, so the ancestor walk would find nothing behind it, fall
 *    through to white, and invent a failure. Skip it, and rely on the content
 ...
 *
 * The default is the whole page, and that is what every full `scan` uses. The
 * narrowed form is what makes this lab's short-lived states measurable at all.
 * Two of them exist: `copyToClipboard` swaps a button's label to
 * "Copied!"/"Failed"/"Unavailable" for 1200ms, and `#btn-decrypt` adds
 * `.recovered` to the decrypted box for 600ms. Both are shorter than a full
 * axe pass, so a whole-page scan would catch them only sometimes, and a
 * sometimes-measured assertion is worse than none. Scoped to one subtree the
 * walk costs a couple of milliseconds, so `expectTransientContrast` in
 * `gate.ts` can set the state, measure, and confirm the state is still set.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * `color-mix(in oklab, var(--accent) 6%, transparent)` and its label ink is
     * `color-mix(in oklab, var(--accent) 70%, var(--text))`, and Chromium
     * reports both to `getComputedStyle` unchanged rather than converting them
     * to sRGB. A regex that only understands `rgb()/rgba()` sees `null` for
     * each and the walk then falls through to the wrong backdrop — which
     ...
     *
     * THIS LAB PAINTS NO GRADIENT TODAY. Every surface it defines is a flat
     * `background-color`, and the only translucency is `rgba()` tints, which
     * `paintAt` handles before it ever reaches here. The sampler is kept
     * anyway, and not because it might be useful later: `paintAt` has to decide
     * what a `background-image` layer contributes, and the alternative default
     * — treat it as transparent — is silently wrong the moment anyone adds a
     * gradient band to the XOR exhibit or the keystream grid. The walk would
     * then composite the text onto the surface BEHIND the gradient and report a
     ...
     *
     * A driven scan here walks a 129-cell XOR grid (three rows of one column
     * per plaintext byte, each column holding a glyph span and a hex span), a
     * 64-cell keystream grid, two more 64-cell avalanche grids, a 16-cell state
     * matrix and a 4-row quarter-round table — several hundred text-owning
     * nodes, every one of which re-walks the same handful of ancestors up to
     ...
     * configuration across four configurations. Nothing mutates the DOM during
     ...
     * that is `.xor-viz`, `#qr-step-table`, `.table-scroll`, `.info-tabs` and
     * the `overflow: hidden` `.qr-progress` track.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This lab draws no SVG figure at all, so today the
      // filter is inert; it is the guard that keeps the first one honest.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab hits that case
      // twice over: the "Why this matters" <details> is shut on arrival (and
      // the gate opens it by clicking its <summary>, the route a reader has,
      // rather than setting `.open` from script, which is what the previous
      // gate did), and the three `.info-tab-panel`s are toggled by an `.active`
      // class whose inactive form is `display: none`.
     ...
     * this page. Everything `aria-hidden` here is genuinely decorative and
     * duplicated in exposed text: the section-heading emoji `.icon` spans, the
     * shared header's hamburger and GitHub glyphs, and `.ac-op`'s
     * "→ flip 1 bit →" arrow, whose meaning is restated verbatim in the
     * `role="status"` `.ac-caption` under it. `#keystream-grid`, `.ac-grid` and
     * the state matrix are NOT hidden — they carry live hex output, and they
     * are measured.
     */
     ...
     * cheap to make and silent, and `ui.ts` builds the state matrix, the
     * quarter-round table and the crib-drag output by interpolating into
     * template literals, which is exactly where it happens: interpolating a
     * string ARRAY into a template literal (`<g>${dots}</g>`) makes JS join it
     * with commas, leaving a run of "," text nodes inside the group. They are
     * invisible on screen, but a walk that measures them reports a 1:1 failure
     * against the section card — a ratio describing ink never laid down.
     */
